Ext.define("App.controller.vasilhame.entregues.itens.baixas.EntregueItemBaixaRel", {
    extend: "Ext.app.Controller", 
    views: [
        "vasilhame.entregues.itens.baixas.relatorio.Window",
        "vasilhame.entregues.itens.baixas.relatorio.Form"
    ],
    
    init: function() {
        this.control ({
            "entregueItemBaixaRelWindow": {
                show: this.onReady
            },
            
            "entregueItemBaixaRelForm > relatoriosTopBar button[name=btnConfirmar]": {
                click: this.botaoTopBarConfirmar
            },
            "entregueItemBaixaRelForm > relatoriosTopBar button[name=btnOrdenar]": {
                click: this.botaoTopBarOrdenar
            },
            "entregueItemBaixaRelForm > relatoriosTopBar button[name=btnAjuda]": {
                click: this.botaoTopBarAjuda
            },
            "entregueItemBaixaRelForm > relatoriosTopBar button[name=btnFechar]": {
                click: this.botaoTopBarFechar
            },
            "entregueItemBaixaRelForm textfield[name=idEmpresa]": {
                focus: this.onFocus,
                blur: this.empresaBlur
            },
            "entregueItemBaixaRelForm button[name=conEmpresas]": {
                click: this.conEmpresasClick
            },
            "entregueItemBaixaRelForm textfield[name=idFilial]": {
                focus: this.onFocus,
                blur: this.filialBlur
            },
            "entregueItemBaixaRelForm button[name=conFiliais]": {
                click: this.conFiliaisClick
            },
            "entregueItemBaixaRelForm textfield[name=idVasilhameEntregue]": {
                focus: this.onFocus,
                blur: this.vasilhameBlur
            },
            "entregueItemBaixaRelForm button[name=conVasilhames]": {
                click: this.conVasilhamesClick
            },
            "entregueItemBaixaRelForm textfield[name=idPessoa]": {
                focus: this.onFocus,
                blur: this.pessoaBlur
            },
            "entregueItemBaixaRelForm button[name=conPessoas]": {
                click: this.conPessoasClick
            },
            "entregueItemBaixaRelForm textfield[name=idUsuarioEntregue]": {
                focus: this.onFocus,
                blur: this.usuarioEntregueBlur
            },
            "entregueItemBaixaRelForm button[name=conUsuariosEntregue]": {
                click: this.conUsuariosEntregueClick
            },
            "entregueItemBaixaRelForm textfield[name=idProduto]": {
                focus: this.onFocus,
                blur: this.produtoBlur
            },
            "entregueItemBaixaRelForm button[name=conProdutos]": {
                click: this.conProdutosClick
            },
            "entregueItemBaixaRelForm textfield[name=idVasilhameEntregueItem]": {
                focus: this.onFocus,
                blur: this.vasilhameItemBlur
            },
            "entregueItemBaixaRelForm button[name=conVasilhamesItens]": {
                click: this.conVasilhamesItensClick
            },
            "entregueItemBaixaRelForm textfield[name=idUsuario]": {
                focus: this.onFocus,
                blur: this.usuarioBlur,
                keydown: this.usuarioKeyDown
            },
            "entregueItemBaixaRelForm button[name=conUsuarios]": {
    ...
    onReady: function(window){
        var form = window.down("entregueItemBaixaRelForm");
    ...
    botaoTopBarConfirmar: function(button) {
        var me = this;
        var form = button.up("entregueItemBaixaRelForm");
    ...
    botaoTopBarOrdenar: function(button) {
        ...
        if (!(grid.storeOrdCampos)) {
            ...
            grid.storeOrdCampos.dados = [
                ["idVasilhameEntregueItem", "ASC", "ID Vasilhame Item"],
    ...
    empresaBlur: function(field) {
        if (field.getValue() != field.oldValue) {
            var me = this;
            var form = field.up("entregueItemBaixaRelForm");
    ...
    carregaRegistro: function(form, registros) {
        ...
        if (erros.length > 0) {
            ...
            if (formato == "PDF") {
                url = BACKEND + "/vasilhame/entregues/itens/baixas/relatorio/pdf";
            } else if (formato == "TXT") {
                url = BACKEND + "/vasilhame/entregues/itens/baixas/relatorio/txt";
            }      
            ...
            callReport.nameFile = "Baixas dos Itens dos Vasilhames Entregues." + formato.toLowerCase();
    ...
    carregaEmpresa: function(form, registro) {
        var request = {
            async: true,
            showMsgErrors: false,
            url: BACKEND + "/vasilhame/entregues/itens/baixas/cadastro/exists/empresa",
    ...
    carregaFilial: function(form, registro) {
        var request = {
            async: true,
            showMsgErrors: false,
            url: BACKEND + "/vasilhame/entregues/itens/baixas/cadastro/exists/filial",
            parametros: Ext.JSON.encode({
                registro: registro
            }),
