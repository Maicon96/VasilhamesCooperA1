Ext.define("App.controller.vasilhame.entregar.EntregarRel", {
    extend: "Ext.app.Controller", 
    views: [
        "vasilhame.entregar.relatorio.Window",
        "vasilhame.entregar.relatorio.Form"
    ],
    ...
    init: function() {
        this.control ({
            ...
            "entregarRelForm > relatoriosTopBar button[name=btnConfirmar]": {
                click: this.botaoTopBarConfirmar
            },
            "entregarRelForm > relatoriosTopBar button[name=btnOrdenar]": {
                click: this.botaoTopBarOrdenar
            },
            "entregarRelForm > relatoriosTopBar button[name=btnAjuda]": {
                click: this.botaoTopBarAjuda
            },
            "entregarRelForm > relatoriosTopBar button[name=btnFechar]": {
                click: this.botaoTopBarFechar
            },
            "entregarRelForm textfield[name=idEmpresa]": {
                focus: this.onFocus,
                blur: this.empresaBlur
            },
            "entregarRelForm button[name=conEmpresas]": {
                click: this.conEmpresasClick
            },
            "entregarRelForm textfield[name=idFilial]": {
                focus: this.onFocus,
                blur: this.filialBlur
            },
            "entregarRelForm button[name=conFiliais]": {
                click: this.conFiliaisClick
            },
            "entregarRelForm textfield[name=idPessoa]": {
                focus: this.onFocus,
                blur: this.pessoaBlur
            },
            "entregarRelForm button[name=conPessoas]": {
                click: this.conPessoasClick
            },
            "entregarRelForm textfield[name=idUsuario]": {
                focus: this.onFocus,
                blur: this.usuarioBlur,
                keydown: this.usuarioKeyDown
            },
            "entregarRelForm button[name=conUsuarios]": {
    ...
    onReady: function(window){
        var form = window.down("entregarRelForm");
    ...
    botaoTopBarConfirmar: function(button) {
        var me = this;
        var form = button.up("entregarRelForm");
    ...
    empresaBlur: function(field) {
        if (field.getValue() != field.oldValue) {
            var me = this;
            var form = field.up("entregarRelForm");
    ...
    carregaRegistro: function(form, registros) {
        ...
        if (erros.length > 0) {
            ...
            if (formato == "PDF") {
                url = BACKEND + "/vasilhame/entregar/relatorio/pdf";
            } else if (formato == "TXT") {
                url = BACKEND + "/vasilhame/entregar/relatorio/txt";
            }      
            ...
            callReport.nameFile = "Vasilhames a Entregar." + formato.toLowerCase();
    ...
    carregaEmpresa: function(form, registro) {
        var request = {
            async: true,
            showMsgErrors: false,
            url: BACKEND + "/vasilhame/entregar/cadastro/exists/empresa",
    ...
    carregaFilial: function(form, registro) {
        var request = {
            async: true,
            showMsgErrors: false,
            url: BACKEND + "/vasilhame/entregar/cadastro/exists/filial",
            parametros: Ext.JSON.encode({
                registro: registro
            }),
