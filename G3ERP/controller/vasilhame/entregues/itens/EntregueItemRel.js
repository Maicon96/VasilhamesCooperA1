Ext.define("App.controller.vasilhame.entregues.itens.EntregueItemRel", {
    extend: "Ext.app.Controller", 
    views: [
        "vasilhame.entregues.itens.relatorio.Window",
        "vasilhame.entregues.itens.relatorio.Form"
    ],
    
    init: function() {
        this.control ({
            "entregueItemRelWindow": {
                show: this.onReady
            },
            
            "entregueItemRelForm > relatoriosTopBar button[name=btnConfirmar]": {
                click: this.botaoTopBarConfirmar
            },
            "entregueItemRelForm > relatoriosTopBar button[name=btnOrdenar]": {
                click: this.botaoTopBarOrdenar
            },
            "entregueItemRelForm > relatoriosTopBar button[name=btnAjuda]": {
                click: this.botaoTopBarAjuda
            },
            "entregueItemRelForm > relatoriosTopBar button[name=btnFechar]": {
                click: this.botaoTopBarFechar
            },
            "entregueItemRelForm textfield[name=idEmpresa]": {
                focus: this.onFocus,
                blur: this.empresaBlur
            },
            "entregueItemRelForm button[name=conEmpresas]": {
                click: this.conEmpresasClick
            },
            "entregueItemRelForm textfield[name=idFilial]": {
                focus: this.onFocus,
                blur: this.filialBlur
            },
            "entregueItemRelForm button[name=conFiliais]": {
                click: this.conFiliaisClick
            },
            "entregueItemRelForm textfield[name=idVasilhameEntregue]": {
                focus: this.onFocus,
                blur: this.vasilhameBlur
            },
            "entregueItemRelForm button[name=conVasilhames]": {
                click: this.conVasilhamesClick
            },
            "entregueItemRelForm textfield[name=idPessoa]": {
                focus: this.onFocus,
                blur: this.pessoaBlur
            },
            "entregueItemRelForm button[name=conPessoas]": {
                click: this.conPessoasClick
            },
            "entregueItemRelForm textfield[name=idUsuarioEntregue]": {
                focus: this.onFocus,
                blur: this.usuarioEntregueBlur
            },
            "entregueItemRelForm button[name=conUsuariosEntregue]": {
                click: this.conUsuariosEntregueClick
            },
            "entregueItemRelForm textfield[name=idProduto]": {
                focus: this.onFocus,
                blur: this.produtoBlur
            },
            "entregueItemRelForm button[name=conProdutos]": {
                click: this.conProdutosClick
            },
            "entregueItemRelForm textfield[name=idUsuario]": {
                focus: this.onFocus,
                blur: this.usuarioBlur,
                keydown: this.usuarioKeyDown
            },
            "entregueItemRelForm button[name=conUsuarios]": {
    ...
    onReady: function(window){
        var form = window.down("entregueItemRelForm");
    ...
    botaoTopBarConfirmar: function(button) {
        var me = this;
        var form = button.up("entregueItemRelForm");
    ...
    botaoTopBarOrdenar: function(button) {
        ...
        if (!(grid.storeOrdCampos)) {
            ...
            grid.storeOrdCampos.dados = [
                ["idVasilhameEntregue", "ASC", "Produto"],
                ["idProduto", "ASC", "Produto"],
                ["quantidade", "ASC", "Quantidade"],
                ["garrafeira", "ASC", "Garrafeira"],
            ...
            grid.storeOrdCampos.loadData([
                ...
                ["idProduto", "ASC", "Produto"],
                ["quantidade", "ASC", "Quantidade"],
                ["garrafeira", "ASC", "Tipo Garrafeira"],
                ["situacao", "ASC", "Situação"],
                ["dataHoraGravacao", "DESC", "Data/Hora Gravação"],
                ["idUsuario", "ASC", "Usuário"]
            ], false);
    ...
    empresaBlur: function(field) {
        if (field.getValue() != field.oldValue) {
            var me = this;
            var form = field.up("entregueItemRelForm");
    ...
    filialBlur: function(field) {
        if (field.getValue() != field.oldValue) {
            var me = this;
            var form = field.up("entregueItemRelForm");
            if ( (isNaN(field.getValue())) || (field.getValue() == "") || (field.getValue() == 0) ) {
    ...
    carregaRegistro: function(form, registros) {
        ...
        if (erros.length > 0) {
            ...
            if (formato == "PDF") {
                url = BACKEND + "/vasilhame/entregues/itens/relatorio/pdf";
            } else if (formato == "TXT") {
                url = BACKEND + "/vasilhame/entregues/itens/relatorio/txt";
            }      
            ...
            callReport.nameFile = "Itens dos Vasilhames Entregues." + formato.toLowerCase();
    ...
    carregaEmpresa: function(form, registro) {
        var request = {
            async: true,
            showMsgErrors: false,
            url: BACKEND + "/vasilhame/entregues/itens/cadastro/exists/empresa",
    ...
    carregaFilial: function(form, registro) {
        var request = {
            async: true,
            showMsgErrors: false,
            url: BACKEND + "/vasilhame/entregues/itens/cadastro/exists/filial",
            parametros: Ext.JSON.encode({
                registro: registro
            }),
...
//                form.getForm().findField("descricaoFilial").setValue("");
    ...
    carregaPessoa: function (form, registro) {
        var request = {
            async: true,
            showMsgErrors: false,
            url: BACKEND + "/vasilhame/entregue/cadastro/exists/pessoa",
            parametros: Ext.JSON.encode({
                registro: registro
            }),
    ...
    carregaUsuarioEntregue: function(form, registro) {
        var request = {
            async: true,
            showMsgErrors: false,
            url: BACKEND + "/vasilhame/entregue/cadastro/exists/usuario",
            parametros: Ext.JSON.encode({
                registro: registro
            }),
