Ext.define("App.controller.vasilhame.entregues.itens.baixas.EntregueItemBaixaCon", {
    extend: "Ext.app.Controller", 
    views: [
        "vasilhame.entregues.itens.baixas.consulta.Grid",
        "vasilhame.entregues.itens.baixas.consulta.Window"
    ],

    init: function() {
        this.control ({
            "entregueItemBaixaConWindow": {
                show: this.onReady,
                destroy: this.onExit
            },
            
            "entregueItemBaixaConGrid": {
                itemdblclick: this.itemDuploClick
            },
            "entregueItemBaixaConGrid > consultasTopBar button[name=btnConfirmar]": {
                click: this.botaoTopBarConfirmar
            },
            "entregueItemBaixaConGrid > consultasTopBar button[name=btnNovo]": {
                click: this.botaoTopBarNovo
            },
            "entregueItemBaixaConGrid > consultasTopBar button[name=btnAlterar]": {
                click: this.botaoTopBarAlterar
            },
            "entregueItemBaixaConGrid > consultasTopBar button[name=btnExcluir]": {
                click: this.botaoTopBarExcluir
            },
            "entregueItemBaixaConGrid > consultasTopBar button[name=btnFiltroPadrao]": {
                click: this.botaoTopBarFiltroPadrao
            },
            "entregueItemBaixaConGrid > consultasTopBar button[name=btnFiltroAvancado]": {
                click: this.botaoTopBarFiltrosAvancados
            },
            "entregueItemBaixaConGrid > consultasTopBar button[name=btnLimpar]": {
                click: this.botaoTopBarLimpar
            },
            "entregueItemBaixaConGrid > consultasTopBar button[name=btnOrdenar]": {
                click: this.botaoTopBarOrdenar
            },
            "entregueItemBaixaConGrid > consultasTopBar button[name=btnAjuda]": {
                click: this.botaoTopBarAjuda
            },
            "entregueItemBaixaConGrid > consultasTopBar button[name=btnFechar]": {
    ...
    botaoTopBarConfirmar: function(button) {
        ...
        var grid = button.up("entregueItemBaixaConGrid");
                ...
                afterClickBtnYES: function(btn) {
                    ...
                    for (var i=0; i<records.length; i++) {
                        ...
                        registros.push({
                            ...
                            idFilial: records[i].data.idFilial,
                            idCodigo: records[i].data.idCodigo,
                            tipoBaixa: records[i].data.tipoBaixa,
                            idVasilhameEntregueItem: records[i].data.idVasilhameEntregueItem                          
                        });
                    }
    ...
    chamaProgramaManutencao: function(grid, record) {
        ...
        callProgram.programa = "vasilhames.EntregueItemBaixaCad";
        ...
        if (record) {
            callProgram.chave = {
                ...
                idVasilhameEntregueItem: record.data.idVasilhameEntregueItem
            };
        }         
    ...
    excluirRegistro: function(grid, registros) {
        var request = {
            async: true,
            showMsgErrors: true,
            url: BACKEND + "/vasilhame/entregues/itens/baixas/consulta/delete",
