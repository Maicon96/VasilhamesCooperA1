Ext.define("App.controller.vasilhame.entregar.EntregarCad", {
    extend: "Ext.app.Controller", 
    views: [
        "vasilhame.entregar.cadastro.Window",
        "vasilhame.entregar.cadastro.Form",
        "vasilhame.entregar.cadastro.ButtonGroup",
        "vasilhame.entregar.cadastro.TabPanel",
        "vasilhame.entregar.cadastro.Itens"
    ],
    ...
    init: function() {
        this.control ({
            ...
            "entregarCadForm > edicoesTopBar button[name=btnNovo]": {
                click: this.botaoTopBarNovo
            },
            "entregarCadForm > edicoesTopBar button[name=btnSalvar]": {
                click: this.botaoTopBarSalvar
            },
            "entregarCadForm > edicoesTopBar button[name=btnExcluir]": {
                click: this.botaoTopBarExcluir,
                disable: this.botaoTopBarStatus,
                enable: this.botaoTopBarStatus
            },
            "entregarCadForm > edicoesTopBar button[name=btnFinalizar]": {
                click: this.botaoTopBarFinalizar
            },
            "entregarCadForm > edicoesTopBar button[name=btnAbrir]": {
                click: this.botaoTopBarAbrir
            },
            "entregarCadForm > edicoesTopBar button[name=btnInutilizar]": {
                click: this.botaoTopBarInutilizar
            },
            "entregarCadForm > edicoesTopBar button[name=btnPDF]": {
                click: this.botaoTopBarPDF
            },
            "entregarCadForm > edicoesTopBar button[name=btnAjuda]": {
                click: this.botaoTopBarAjuda
            },
            "entregarCadForm > edicoesTopBar button[name=btnFechar]": {
                click: this.botaoTopBarFechar
            },
            "entregarCadForm textfield[name=idEmpresa]": {
                focus: this.onFocus,
                blur: this.empresaBlur
            },
            "entregarCadForm button[name=conEmpresas]": {
                click: this.conEmpresasClick
            },
            "entregarCadForm textfield[name=idFilial]": {
                focus: this.onFocus,
                blur: this.filialBlur
            },
            "entregarCadForm button[name=conFiliais]": {
                click: this.conFiliaisClick
            },
            "entregarCadForm textfield[name=idPessoa]": {
                focus: this.onFocus,
                blur: this.pessoaBlur,
                change: this.pessoaChange
            },
            "entregarCadForm button[name=conPessoas]": {
                click: this.conPessoasClick
            },
            "entregarCadForm textfield[name=idUsuario]": {
                focus: this.onFocus,
                blur: this.usuarioBlur
            },
            "entregarCadForm button[name=conUsuarios]": {
                click: this.conUsuariosClick
            },
            "entregarCadForm textfield[name=observacao]": {
                focus: this.onFocus,
                keydown: this.observacaoKeyDown
            },
            
            "entregarCadForm button[name=item]": {
                click: this.botaoBotBarItemClick
            },
            "entregarCadForm menuitem[name=item]": {
                click: this.botaoBotBarItemClick
            }
        });
    },
    ...
    onReady: function(window) {        
        var me = this;
        var form = window.down("entregarCadForm");
    ...
    botaoTopBarNovo: function(button){
        var me = this;
        var form = button.up("entregarCadForm");
         ...
         if (situacao != 2) {
            if (situacao == 1) {    
                ...
                var request = {
                    async: true,
                    showMsgErrors: true,
                    url: BACKEND + "/vasilhame/entregar/cadastro/finalizar",
...
//                    },
                ...
                callMessageBox.msg = "Não foi possível finalizar o vasilhame a entregar, situação precisa ser aberto";
    ...
    botaoTopBarAbrir: function(button) {
        ...
        if (situacao != 1) {
            ...
            var request = {
                async: true,
                showMsgErrors: true,
                url: BACKEND + "/vasilhame/entregar/cadastro/abrir",
                parametros: Ext.JSON.encode({
                    registro: registro
                }),
                component: form,
            ...
            callMessageBox.show({
                afterClickBtnYES: function(btn) {
                    ...
                    var request = {
                        async: true,
                        showMsgErrors: true,
                        url: BACKEND + "/vasilhame/entregar/cadastro/inutilizar",
                        parametros: Ext.JSON.encode({
                            registro: registro
                        }),
                        component: form,
...
//                        },
    ...
    botaoTopBarPDF: function(button) {
        ...
        if (situacao != 1) {
            ...
            var chave = {
                vasilhameEntregar: vasilhameEntregar            
            };                             

            var formato = "PDF";    
            var url = BACKEND + "/vasilhame/entregar/comprovante/relatorio/pdf";        

            var callReport = new CallReport1();
            callReport.url = url;        
            callReport.format = formato;
            callReport.nameFile = "Vasilhames a Entregar Comprovante." + formato.toLowerCase();
    ...
    empresaBlur: function(field) {
        if (field.getValue() != field.oldValue) {
            var me = this;
            var form = field.up("entregarCadForm");
    ...
    botaoBotBarItemClick: function(button) {  
        ...
        if (button.id.indexOf("menuitem") !== -1) {
            button = button.up("entregarCadButtonGroup").down("toolbar").down("button[name=" + button.name + "]");
        }
        ...
        if (button.up("entregarCadForm").params) {
            if (button.up("entregarCadForm").params.loadParents) {
                callProgram.loadParents = button.up("entregarCadForm").params.loadParents;
            }
        }
    ...
    limparTela: function(form, ignoreFields) {
        ...
        if (form.modoCompl == "abas") {
            var gridItem = form.down("entregarItemConGrid");            
        ...
        if (form.modoCompl == "botoes") {
            form.down("entregarCadButtonGroup").down("toolbar").disable();            
        }
    },
    ...
    carregaRegistro: function(form, registro, focus) {
        ...
        var request = {
            async: true,
            showMsgErrors: false,
            url: BACKEND + "/vasilhame/entregar/cadastro/exists",
            parametros: Ext.JSON.encode({
                registro: registro
            }),
            component: form,
            ...
            afterMsgSuccessTrue: function(resposta, request) {
                ...
                if (form.modoCompl == "botoes") {
                    form.down("entregarCadButtonGroup").down("toolbar").enable();
                }            
    ...
    gravaRegistro: function(form, registro) {
        ...
        if (erros.length > 0) {
            ...
            var url = "/vasilhame/entregar/cadastro/save";
            if (registro.idCodigo) {
                url = "/vasilhame/entregar/cadastro/update";
            }
    ...
    excluirRegistro: function(form, registros) {        
        ...
        var request = {
            async: true,
            showMsgErrors: true,
            url: BACKEND + "/vasilhame/entregar/cadastro/delete",
            parametros: Ext.JSON.encode({
                registros: registros
            }),
            component: form,
    ...
    carregaPessoa: function (form, registro) {        
        var url = BACKEND + "/vasilhame/entregar/cadastro/exists/pessoa";
        if (form.down("edicoesTopBar button[name=btnExcluir]").disabled) {
          url = BACKEND + "/vasilhame/entregar/cadastro/exists/pessoa/ativa";
        }                
    ...
    carregaRegistrosLoadParents: function(form, registro) {
        ...
        var request = {
            async: true,
            showMsgErrors: false,
            url: BACKEND + "/vasilhame/entregar/cadastro/consultasituacao",
            parametros: Ext.JSON.encode({
                registro: registro
            }),
            component: form,
    ...
    filtros: function (form, idEmpresa, nomeEmpresa, idFilial, descricaoFilial, idVasilhameEntregar, idPessoa,
        ...
        if (form.modoCompl == "abas") {            
            ...
            if (!form.down("edicoesTopBar button[name=btnExcluir]").disabled) {
                if (form.down("entregarCadTabPanel").activeTab.xtype == "entregarCadItem") {
        ...
        if (form.modoCompl == "botoes") {
            form.down("entregarCadButtonGroup").down("toolbar").down("button[name=item]").fieldsFix = filtrosItens;
        }
    }
