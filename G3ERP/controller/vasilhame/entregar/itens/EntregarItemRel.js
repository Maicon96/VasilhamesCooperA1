Ext.define("App.controller.vasilhame.entregar.itens.EntregarItemRel", {
    extend: "Ext.app.Controller", 
    views: [
        "vasilhame.entregar.itens.relatorio.Window",
        "vasilhame.entregar.itens.relatorio.Form"
    ],
    
    init: function() {
        this.control ({
            "entregarItemRelWindow": {
                show: this.onReady
            },
            
            "entregarItemRelForm > relatoriosTopBar button[name=btnConfirmar]": {
                click: this.botaoTopBarConfirmar
            },
            "entregarItemRelForm > relatoriosTopBar button[name=btnOrdenar]": {
                click: this.botaoTopBarOrdenar
            },
            "entregarItemRelForm > relatoriosTopBar button[name=btnAjuda]": {
                click: this.botaoTopBarAjuda
            },
            "entregarItemRelForm > relatoriosTopBar button[name=btnFechar]": {
                click: this.botaoTopBarFechar
            },
            "entregarItemRelForm textfield[name=idEmpresa]": {
                focus: this.onFocus,
                blur: this.empresaBlur
            },
            "entregarItemRelForm button[name=conEmpresas]": {
                click: this.conEmpresasClick
            },
            "entregarItemRelForm textfield[name=idFilial]": {
                focus: this.onFocus,
                blur: this.filialBlur
            },
            "entregarItemRelForm button[name=conFiliais]": {
                click: this.conFiliaisClick
            },
            "entregarItemRelForm textfield[name=idVasilhameEntregar]": {
                focus: this.onFocus,
                blur: this.vasilhameBlur
            },
            "entregarItemRelForm button[name=conVasilhames]": {
                click: this.conVasilhamesClick
            },
            "entregarItemRelForm textfield[name=idPessoa]": {
                focus: this.onFocus,
                blur: this.pessoaBlur
            },
            "entregarItemRelForm button[name=conPessoas]": {
                click: this.conPessoasClick
            },
            "entregarItemRelForm textfield[name=idUsuarioEntregar]": {
                focus: this.onFocus,
                blur: this.usuarioEntregarBlur
            },
            "entregarItemRelForm button[name=conUsuariosEntregar]": {
                click: this.conUsuariosEntregarClick
            },
            "entregarItemRelForm textfield[name=idProduto]": {
                focus: this.onFocus,
                blur: this.produtoBlur
            },
            "entregarItemRelForm button[name=conProdutos]": {
                click: this.conProdutosClick
            },
            "entregarItemRelForm textfield[name=idUsuario]": {
                focus: this.onFocus,
                blur: this.usuarioBlur,
                keydown: this.usuarioKeyDown
            },
            "entregarItemRelForm button[name=conUsuarios]": {
    ...
    onReady: function(window){
        var form = window.down("entregarItemRelForm");
    ...
    botaoTopBarConfirmar: function(button) {
        var me = this;
        var form = button.up("entregarItemRelForm");
    ...
    botaoTopBarOrdenar: function(button) {
        ...
        if (!(grid.storeOrdCampos)) {
            ...
            grid.storeOrdCampos.dados = [
                ["idVasilhameEntregar", "ASC", "Vasilhame"],
    ...
    empresaBlur: function(field) {
        if (field.getValue() != field.oldValue) {
            var me = this;
            var form = field.up("entregarItemRelForm");
    ...
    carregaRegistro: function(form, registros) {
        ...
        if (erros.length > 0) {
            ...
            if (formato == "PDF") {
                url = BACKEND + "/vasilhame/entregar/itens/relatorio/pdf";
            } else if (formato == "TXT") {
                url = BACKEND + "/vasilhame/entregar/itens/relatorio/txt";
            }      
            ...
            callReport.nameFile = "Itens dos Vasilhames a Entregar." + formato.toLowerCase();
    ...
    carregaEmpresa: function(form, registro) {
        var request = {
            async: true,
            showMsgErrors: false,
            url: BACKEND + "/vasilhame/entregar/itens/cadastro/exists/empresa",
    ...
    carregaFilial: function(form, registro) {
        var request = {
            async: true,
            showMsgErrors: false,
            url: BACKEND + "/vasilhame/entregar/itens/cadastro/exists/filial",
            parametros: Ext.JSON.encode({
                registro: registro
            }),
